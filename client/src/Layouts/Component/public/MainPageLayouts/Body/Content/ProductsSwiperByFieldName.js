...
const ProductsSwiperByFieldName = ({ category, dispatch }) => {
    const [products, setProducts] = useState(null);
    const [isLoading, setIsLoading] = useState(false);

    const fetchProducts = async () => {
        setIsLoading(true);
        const response = await apiGetProducts({ mainCategory: category?.title, limit: 10 });
        setIsLoading(false);
    ...
    const handleClickTitle = () => {
        dispatch(filterProducts({ filterProductValue: { mainCategory: category?.title } }));
    };
            ...
            <section>
                <div className="relative desktop:w-main laptop:w-full max-laptop:px-4">
                    <div className="flex justify-start items-center border-b-[3px] border-primary mb-12 w-full mx-auto">
                        ...
                        >
                            <img src={Images.CategoryProductIcon} alt="" className="h-8 w-8 object-cover"></img>
...
export default withComponent(memo(ProductsSwiperByFieldName));
